...
def train(opt):
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        for i, data in enumerate(dataset, start=epoch_iter):
            ...
            disc_losses = model.update_GGDC_D()
            gen_losses, gen_in, gen_out, rec_out, cyc_out = model.update_GGDC_G(infer=save_fake)
            ...
            if total_steps % opt.print_freq == print_delta:
                errors = {k: v.item() if not (isinstance(v, float) or isinstance(v, int)) else v for k, v in
                          loss_dict.items()}
        ...
        print('End of epoch %d / %d \t Time Taken: %d sec' %
              (epoch + 1, opt.epochs, iter_end_time - epoch_start_time))
