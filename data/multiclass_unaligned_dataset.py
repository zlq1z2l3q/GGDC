...
class MulticlassUnalignedDataset(BaseDataset):
        ...
        if not self.in_the_wild:
            ...
            for currDir in subDirs:
                if self.opt.isTrain:
                    ...
                    prefix = 'test'
                if prefix in currDir:  # we assume that the class name starts with the 'prefix' ('train' or 'test')
    ...
    def __getitem__(self, index):
        if self.opt.isTrain and not self.get_samples:  # train mode
            condition = True
            ...
            A_parsing = np.array(A_parsing.getdata(), dtype=np.uint8).reshape(A_parsing.size[1], A_parsing.size[0],
                                                                              3)
            ...
            B_parsing = np.array(B_parsing.getdata(), dtype=np.uint8).reshape(B_parsing.size[1], B_parsing.size[0],
                                                                              3)
            ...
            A_landmarks_path = self.landmark_paths[self.class_A_idx][
                random.randint(0, self.sizes[self.class_A_idx] - 1)]
            A_landmarks = Image.open(A_landmarks_path).convert('L')
            A_landmarks = self.transform(A_landmarks)

            B_landmarks_path = self.landmark_paths[self.class_B_idx][
                random.randint(0, self.sizes[self.class_B_idx] - 1)]
            B_landmarks = Image.open(B_landmarks_path).convert('L')
