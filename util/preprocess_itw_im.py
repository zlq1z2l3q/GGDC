...
class preprocessInTheWildImage:
    ...
    def align_in_the_wild_image(self, np_img, lm, transform_size=4096, enable_padding=True):
        ...
        x /= np.hypot(*x)
        x *= max(np.hypot(*eye_to_eye) * 2.0, np.hypot(
            *eye_to_mouth) * 2.2)  # This results in larger crops then the original FFHQ. For the original crops, replace 2.2 with 1.8
        y = np.flipud(x) * [-1, 1]
